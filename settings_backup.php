...
<head>
    ...
    <script>
        function validatebackupForm() {
            var password = document.forms["backup-Form"]["password"].value;
            ...
            if (password==''){
                ...
                document.getElementById('error-password').style.color = "";
            }
            if ($_SESSION['']){
...
<div class="">
    ...
    <div class="bg-dark border- h-100 w-25 fixed-top">
...
<div class="container-fluid fm_form2_container rounded-lg shadow-sm pb-2">
    ...
    <ul class="nav nav-pills mt-2">
        ...
        <li class="nav-item shadow-sm w-25"><a class="nav-link active btn-secondary" href="settings_backup.php">Backup</a></li>
    </ul>
    <!--backup container-->
    <div class="border rounded-sm mb-4 p-4 mt-5">
        <form class="form-group mt-4 mb-4" method="post" action="backupDatabase.php" name="backup-Form" onsubmit="return validatebackupForm()">
            <!--Password error message-->
            <?php if (isset($_SESSION['passwordError_backup'])): ?>
                <div class="mt-3 ml-3 mr-3 alert alert-danger">
                    <?php
                    echo $_SESSION['passwordError_backup'];
                    unset($_SESSION['passwordError_backup']);
                    ?>
                </div>
            <?php endif ?>

            <label for="password" id="error-password">Enter Password</label>
            <input type="password" name="passwordValidation" id="password" size="30" class="rounded-sm  ml-3">
            <br>
            <input type="submit" name="backup_db" value="Backup Database" class="btn-success rounded-lg w-100 p-2 mt-4 ">
        </form>

        <p class="alert alert-warning mt-5"><b>NOTE: </b>The database is downloaded with .sql extension. Backup file will save inside the Download folder.</p>
    </div>
</div>
</body>